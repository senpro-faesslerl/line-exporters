...
echo ${getEndpoints//\\/\\\\} | jq -c '.items[]' | while read endpoint; do
    echo "sophos.central.endpoint host=\"$(echo ${endpoint} | jq '.hostname' | sed 's/[\"]*//g')\",id=\"$(echo ${endpoint} | jq '.id' | sed 's/[\"]*//g')\" healthOverall=$(echo ${endpoint} | jq '.health.overall' | sed 's/[\"]*//g;s/good/0/g;s/suspicious/1/g;s/bad/2/g;s/unknown/3/g')i,threatsStatus=$(echo ${endpoint} | jq '.health.threats.status' | sed 's/[\"]*//g;s/good/0/g;s/suspicious/1/g;s/bad/2/g;s/unknown/3/g')i,servicesStatus=$(echo ${endpoint} | jq '.health.services.status' | sed 's/[\"]*//g;s/good/0/g;s/suspicious/1/g;s/bad/2/g;s/unknown/3/g')i,osIsServer=$(echo ${endpoint} | jq '.os.isServer' | sed 's/[\"]*//g'),osName=\"$(echo ${endpoint} | jq '.os.name' | sed 's/[\"]*//g')\",osBuild=\"$(echo ${endpoint} | jq '.os.build' | sed 's/[\"]*//g')\",associatedPersonName=\"$(echo ${endpoint} | jq '.associatedPerson.name' | sed 's/^\"//;s/\"$//;s/\\\\/\\/g')\",ipv4Addresses=\"$(echo ${endpoint} | jq -r '.ipv4Addresses[]' 2> /dev/null | paste -sd ',')\",tamperProtectionEnabled=$(echo ${endpoint} | jq '.tamperProtectionEnabled' | sed 's/[\"]*//g'),lastSeenAt=\"$(echo ${endpoint} | jq '.lastSeenAt' | sed 's/[\"]*//g')\"" $(date +%s%N)
done
